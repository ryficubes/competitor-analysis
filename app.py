...
def get_cstimer_times(file, event):
  ...
  #with open(file, "r") as f:
    #data = f.read().strip()  # Remove any leading/trailing spaces or newlines
  dictionary = json.loads(data)

  temp1 = dictionary['properties']['sessionData'].strip()
  temp2 = json.loads(temp1)
  j = 1
  for i in range(1,len(temp2.keys())):
    print(temp2[str(i)]['name'])
    if temp2[str(i)]['name'] == event:
      session_name = str('session' + str(j))
      j+=1
    else:
      j+=1
  print(session_name)
